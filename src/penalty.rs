...
static BASE_PENALTY: KeyMap<f64> = KeyMap([
    9.0, 2.0, 2.0, 2.0, 4.0,    4.0, 2.0, 2.0, 2.0, 9.0,
    1.0, 0.0, 0.0, 0.0, 3.0,    3.0, 0.0, 0.0, 0.0, 1.0,
    6.0, 2.0, 2.0, 2.0, 5.0,    5.0, 2.0, 2.0, 2.0, 6.0]);
...
-> Vec<KeyPenalty<'a>>
{
    ...
    // Penalise 8 points for using the same finger twice on different keys.
    // An extra 8 points for using the centre column.
    penalties.push(KeyPenalty {
        name: "same finger",
    });

    // Penalise 2 points for jumping from top to bottom row or from bottom to
    ...
    // Penalise 15 points for jumping from top to bottom row or from bottom to
    ...
    // Penalise 15 points for awkward pinky/ring combination where the pinky
    // reaches above the ring finger, e.g. QA/AQ, PL/LP, ZX/XZ, ;./.; on Qwerty.
    penalties.push(KeyPenalty {
        name: "pinky/ring twist",
    });

    // Penalise 8 points for reversing a roll at the end of the hand, i.e.
    ...
    // Award 0.15 points for rolling outwards.
    penalties.push(KeyPenalty {
        name: "roll out",
    });
...
-> f64
{
    ...
    if curr.hand == old1.hand {
        ...
        if curr.finger == old1.finger && curr.pos != old1.pos {
            let penalty = 8.0 + if curr.center { 8.0 } else { 0.0 }
                              + if old1.center { 8.0 } else { 0.0 };
